...
--Ödeme şekli kredi kartı olan işlem detaylarını listele
SELECT * FROM benzin_istasyonu_verileri
...
GROUP BY plaka, odeme_sekli 
ORDER BY SUM(litre) DESC LIMIT 1;

--Plakasının 2. rakamı 4 olan araçları listele
SELECT plaka FROM benzin_istasyonu_verileri
WHERE plaka LIKE '_4%';

--Ödeme şekli 3. harfi K olan plakaları listele
SELECT plaka, odeme_sekli FROM benzin_istasyonu_verileri
WHERE odeme_sekli LIKE '__k%';

--Aldığı benzin litresi ile ile başlayan plakaları listele
...
--Aldığı yakıtın son harfi A ile L arasındaki harfler olan araçları ve yakıtları listele
SELECT plaka, yakit_turu FROM benzin_istasyonu_verileri
